...
from musicbot.Troll import trolls
...
gameblindtest = False
channelLH = discord.Object(id='255471585153122307')
...
class MusicBot(discord.Client):
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if self.config.now_playing_mentions:
                if gameblindtest:
                    newmsg = '%s - your song is now playing in %s!' % (
                    entry.meta['author'].mention, player.voice_client.channel.name)
                else:
    ...
    async def on_member_join(self, member):
        if member.server.name == 'Lumen Histoire':
            serveur = member.server
            fmt = 'Welcome {0.mention} to {1.name}!'
            await self.send_message(channelLH,fmt.format(member, serveur))
            await self.add_roles(member, discord.Role(server=serveur, id='256212748797476864'))
            await self.send_message(channelLH, "Contacte un modérateur via message privé pour obtenir le rang de Guildeux")
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if 'entries' in info:
            ...
            if gameblindtest:
                reply_text = "Enqueued your song to be played. Position in queue: %s"
            else:
        ...
        if position == 1 and player.is_stopped:
            ...
            if gameblindtest:
                reply_text %= (position, time_until)
            else:
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            if gameblindtest:
                return Response("Bien tenté", delete_after=30)
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel):
        ...
        if skips_remaining <= 0:
            ...
            if gameblindtest:
                return Response("your skip was acknowledged", reply=True,delete_after=20)
            else:
                return Response(
                    'your skip for **{}** was acknowledged.'
    ...
    async def cmd_blindtest(self, channel):
        global gameblindtest

        if gameblindtest:
            gameblindtest = False
            await self.safe_send_message(channel, "Mode blind test désactivé !")
        else:
            gameblindtest = True
            await self.safe_send_message(channel, "Mode blind test activé !")

    async def cmd_troll(self, author, channel, user_mentions):
        """
        Usage:
            {command_prefix}troll [@User]
        Envoie un troll en mentionnant l'utilisateur indiqué en paramètre.
        """
        if not user_mentions:
            user = author
            troll = random.choice(trolls)
            await self.send_message(channel, troll.format(user))
    ...
    async def on_message(self, message):
        ...
        if gameblindtest and message.content.startswith(self.config.command_prefix+'play'):
